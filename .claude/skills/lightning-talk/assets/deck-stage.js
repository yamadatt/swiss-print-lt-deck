 ...
 *      (≤640px), and via the `no-rail` attribute. The T key (or a
 *      __deck_rail_visible message) toggles it per-viewer, persisted to
 *      localStorage. Rail mutations dispatch
 *      a `deckchange`
  ...
  class DeckStage extends HTMLElement {
    ...
    _collectSlides() {
      ...
      this._slides.forEach((slide, i) => {
        ...
        // Authored ".pageno" text (e.g. "02 / 16") is overwritten here, so
        // adding/removing/reordering slides never leaves stale numbers.
        const pageno = slide.querySelector('.pageno');
        if (pageno) pageno.textContent = `${pad2(n)} / ${pad2(total)}`;
      });
    ...
    /** Soft show/hide of the thumbnail rail. Persists per-viewer alongside
     *  rail width; shared by the __deck_rail_visible message and the T key. */
    _setRailVisible(on) {
    ...
    _onKey(e) {
      ...
      if (key === 'ArrowRight' || key === 'PageDown' || key === ' ' || key === 'Spacebar') {
        ...
        // Toggle the thumbnail rail. No-op while the rail is hard-hidden
        // (no-rail attr, presenting, host preview) — those states own it.
        if (this._railEnabled && !this._presenting && !this._previewMode) {
